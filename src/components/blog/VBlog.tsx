...
export default function VBlog(props: BlogProps) {
  ...
  const toBlogHandler = async () => {
    ...
    const currentBlog: BlogProps | undefined = (
      await getDoc(docRef)
    ).data() as BlogProps;
  ...
  return (
    ...
    >
      <div className="flex w-[58%]">
        <img src={props.post.imageSource} alt="" className="flex-1" />
      </div>
      <div className="flex-1 p-6 ">
        ...
        <div className="border-t flex flex-1 justify-between self-end items-center h-[12%]">
          ...
          <div onClick={favoriteHandler}>
            {isFavorite ? (
              <AiFillHeart color={colors.secondary} size={24} />
            ) : (
              <AiOutlineHeart color={colors.secondary} size={24} />
            )}
          </div>
        </div>
      </div>
